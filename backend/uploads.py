...
def insert_future_daily(df, job_id=None, jobs=None):
    # Deduplicate within the file — keep last row for each (date, stock_symbol)
    df = df.drop_duplicates(subset=["date", "stock_symbol"], keep="last")
    _set_job_stage(jobs, job_id, f"Converting {len(df):,} dates...", 47)
    iso_dates = df["date"].apply(to_iso_date)
    ...
    sql = """
        ...
        VALUES %s
        ON CONFLICT (date, stock_symbol) DO UPDATE SET
            future_index = EXCLUDED.future_index,
            open         = EXCLUDED.open,
            high         = EXCLUDED.high,
            low          = EXCLUDED.low,
            close        = EXCLUDED.close
