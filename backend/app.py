...
from screeners import (
    ...
    run_arbitrage_screener, get_available_dates_future, get_date_range_future,
)
...
def api_available_dates(screener):
    ...
    if screener == "future":
        return jsonify({
            "dates": get_available_dates_future(),
            "range": get_date_range_future(),
        })
...
def _parse_console_rows(prefix, form, valid_keys):
    """Parse metric_<prefix>_N / weight_<prefix>_N pairs from the form."""
    rows = []
    i = 0
    while True:
        metric = form.get(f"metric_{prefix}_{i}")
        if metric is None:
            break
        if metric in valid_keys:
            weight_str = form.get(f"weight_{prefix}_{i}", "").strip()
            weight = float(weight_str) if weight_str else None
            rows.append({"metric": metric, "weight": weight})
        i += 1
    return rows


@app.route("/stock-scoring", methods=["GET", "POST"])
@login_required
def stock_scoring():
    options = fetch_filter_options()
    results = None
    info = None
    return_rows, vol_rows = [], []
    form_data = {
        "index_filter": "All", "sector_filter": "All",
        "date_str": "", "min_price": "", "max_price": "",
    }
    form_errors = []
    ...
    if request.method == "POST":
        form_data["index_filter"]  = request.form.get("index_filter", "All")
        form_data["sector_filter"] = request.form.get("sector_filter", "All")
        form_data["date_str"]      = request.form.get("trade_date", "").strip()
        form_data["min_price"]     = request.form.get("min_price", "").strip()
        form_data["max_price"]     = request.form.get("max_price", "").strip()

        trade_date = parse_iso_date(form_data["date_str"])
        if not form_data["date_str"]:
            form_errors.append("Date is required.")
        elif not trade_date:
            form_errors.append("Date must be in YYYY-MM-DD format.")
        ...
        try:
            ...
            form_errors.append("Min Price must be a number.")
        try:
            if form_data["max_price"]:
                max_price = float(form_data["max_price"])
        except ValueError:
            form_errors.append("Max Price must be a number.")

        return_rows = _parse_console_rows("ret", request.form, RETURN_METRICS)
        vol_rows    = _parse_console_rows("vol", request.form, VOL_METRICS)

        if not return_rows:
            form_errors.append("Add at least one metric in the Return Console.")

        if not form_errors:
            results, info = run_stock_scoring(
                form_data["index_filter"], form_data["sector_filter"],
                min_price, max_price, trade_date, return_rows, vol_rows,
            )
...
@app.route("/stock-scoring/download", methods=["POST"])
@login_required
def stock_scoring_download():
    trade_date    = parse_iso_date(request.form.get("trade_date", "").strip())
    index_filter  = request.form.get("index_filter", "All")
    sector_filter = request.form.get("sector_filter", "All")
    min_price = max_price = None
    try:
        v = request.form.get("min_price", "").strip()
        if v: min_price = float(v)
    except ValueError:
        pass
    try:
        v = request.form.get("max_price", "").strip()
        if v: max_price = float(v)
    except ValueError:
        pass
    ...
    if not trade_date or not return_rows:
        flash("Cannot download: missing date or return metrics.", "error")
        return redirect(url_for("stock_scoring"))

    results, _ = run_stock_scoring(
        index_filter, sector_filter, min_price, max_price,
        trade_date, return_rows, vol_rows,
    )

    ret_labels = [RETURN_METRICS[r["metric"]]["label"] for r in return_rows]
    vol_labels = [VOL_METRICS[r["metric"]]["label"]    for r in vol_rows]
    out = io.StringIO()
    writer = csv.writer(out)
    header = (["Rank", "Symbol", "Stock Name", "Sector", "Indexes",
                "Close Price", "Final Score", "Return Score"]
              + ret_labels
              + (["Volatility Score"] + vol_labels if vol_rows else []))
    writer.writerow(header)
    for r in results:
        row = [r["rank"], r["stock_symbol"], r["stock_name"],
               r["sector"], r["indexes"], r["close"],
               r["score"], r["return_score"]]
        row += [r["ret_metrics"].get(x["metric"], "") for x in return_rows]
        if vol_rows:
            row += [r["vol_score"]]
            row += [r["vol_metrics"].get(x["metric"], "") for x in vol_rows]
        writer.writerow(row)

    return Response(
        out.getvalue(),
        mimetype="text/csv",
        headers={"Content-Disposition": "attachment; filename=stock_scoring_results.csv"},
    )


# ----------------- Arbitrage Trade Screener -----------------
@app.route("/arbitrage-screener", methods=["GET", "POST"])
@login_required
def arbitrage_screener():
    results = None
    info = None
    form_data = {"date_str": "", "threshold_str": ""}
    form_errors = []

    if request.method == "POST":
        date_str = request.form.get("trade_date", "").strip()
        threshold_str = request.form.get("threshold", "").strip()
        form_data["date_str"] = date_str
        form_data["threshold_str"] = threshold_str

        trade_date = parse_iso_date(date_str)
        threshold = None
        ...
        if not threshold_str:
            form_errors.append("Threshold % is required.")
        else:
            try:
                threshold = float(threshold_str)
            except ValueError:
                form_errors.append("Threshold must be a number.")

        if not form_errors:
            results, info = run_arbitrage_screener(trade_date, threshold)

    return render_template(
        "arbitrage_screener.html",
        results=results,
        info=info,
        form_data=form_data,
        form_errors=form_errors,
        date_range=get_date_range_future(),
    )


@app.route("/arbitrage-screener/download", methods=["POST"])
@login_required
def arbitrage_screener_download():
    date_str = request.form.get("trade_date", "").strip()
    threshold_str = request.form.get("threshold", "").strip()
    trade_date = parse_iso_date(date_str)
    ...
    if errors:
        flash("Cannot download: " + "; ".join(errors), "error")
        return redirect(url_for("arbitrage_screener"))

    results, _ = run_arbitrage_screener(trade_date, threshold)

    out = io.StringIO()
    writer = csv.writer(out)
    writer.writerow(["Stock Symbol", "Future Index", "Future Close",
                     "Spot Close", "Difference", "Diff %", "Future Date", "Spot Date"])
    for r in results:
        writer.writerow([
            r["stock_symbol"], r["future_index"], r["fut_close"],
            r["spot_close"], r["difference"], r["diff_pct"],
            r["fut_date"], r["spot_date"],
        ])
    return Response(
        out.getvalue(),
        mimetype="text/csv",
        headers={"Content-Disposition":
                 f"attachment; filename=arbitrage_{date_str}.csv"},
    )
...
from scoring import run_stock_scoring, RETURN_METRICS, VOL_METRICS, PRESETS
